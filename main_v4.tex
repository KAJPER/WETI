...
$f_s = \SI{160}{\kilo\hertz}$ wymaga $f_{CLK\_ADC} = 160000 \cdot 13 = \SI{2,08}{\mega\hertz}$,
co przekracza limit dla 10-bit.

Rozwiązanie przyjęte w~projekcie: praca ADC z~$f_{CLK\_ADC} = \SI{2,5}{\mega\hertz}$
(prescaler DIV8, $\SI{20}{\mega\hertz}/8$) przy akceptacji nieznacznej degradacji
INL/DNL (typowo $< 1{,}5$\,LSB wg characterization data w~datasheet, rys.~37-47).
Algorytm Goertzela i~tak wyznacza fazę z~wielu próbek, co uśrednia błędy
pojedynczych konwersji. Alternatywnie, w~Etapie~2 zostanie zbadane próbkowanie
z~$K = 3$ próbki/cykl ($f_s = \SI{120}{\kilo\hertz}$, $N = 12$, $f_{CLK\_ADC} = \SI{1,25}{\mega\hertz}$
--- w~limicie 10-bit) z~odpowiednio zmodyfikowanym współczynnikiem $k$ Goertzela.
...
\begin{itemize}[nosep]
    ...
    \item C2 = \SI{10}{\micro\farad} --- kondensator sprzęgający AC (wejście).
          \textit{Uwaga:} Przy użyciu ceramicznego kondensatora X5R/X7R
          w~obudowie 0603 należy liczyć się z~efektem piezoelektrycznym
          (mikrofonia) oraz degradacją pojemności pod napięciem polaryzacji.
          W~Etapie~2 wartość zostanie zredukowana do \SI{1}{\micro\farad}
          ($f_{HP} = 1/(2\pi \cdot 100\text{k} \cdot 1\mu) \approx \SI{1,6}{\hertz}$
          --- nadal daleko poniżej $f_0$) lub zastosowany kondensator
          elektrolityczny SMD..
          \textit{Uwaga:} Kondensator ceramiczny \SI{10}{\micro\farad} 0603 (X5R/X7R)
          wykazuje efekt piezoelektryczny (mikrofonia) oraz degradację pojemności
          z~napięciem polaryzacji. W~prototypie rozważona zostanie zamiana na
          \SI{1}{\micro\farad} ($f_{HP} = 1/(2\pi \cdot 100\text{k} \cdot 1\mu) \approx \SI{1,6}{\hertz}$
          --- nadal daleko poniżej $f_0$) lub zastosowanie kondensatora
          elektrolitycznego SMD.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{l r X}
        ...
        Zegar & \SI{20}{\mega\hertz} & Wewn.\ oscylator ($\pm\SI{2}{\percent}$, kalibr.\ fabrycznie) \\
        ADC & 10-bit, ${\leq}\SI{115}{ksps}$ & Pomiar echa + termistor (limit $f_{CLK\_ADC} = \SI{1,5}{\mega\hertz}$ dla 10-bit) \\
        ...
        $I_{DD}$ active & ${\sim}\SI{10}{\milli\ampere}$ & @ \SI{20}{\mega\hertz}, \SI{5}{\volt} (DS Tab.~36-4) \\
        $I_{DD}$ active & ${\sim}\SI{5,5}{\milli\ampere}$ & @ \SI{10}{\mega\hertz}, \SI{3}{\volt} (estym.\ dla \SI{3,3}{\volt}: ${\sim}\SI{6}{\milli\ampere}$) \\
...
\begin{table}[H]
    ...
    \begin{tabular}{llll}
        ...
        PA5 & Burst TX & TCA0-WO5 & PWM \SI{40}{\kilo\hertz} \\
        PA6 & DAC0 OUT & DAC0 & Próg detekcji echa (ref.\ AC0) \\
        ...
        PB4 & ECHO\_DET & AC0-AINP3 & Wejście komparatora (detekcja progu echa) \\
        PB5 & BAZA\_TX & GPIO output & Sterowanie BC817 (backup / push-pull) \\
...
Sygnał PWM jest wyprowadzany na pin PA5, któremu odpowiada wyjście TCA0-WO5
(patrz tabela I/O Multiplexing w~datasheet, Tab.~5-1). Wymaga to ustawienia
bitu CMP2EN w~rejestrze TCA0.CTRLB (w~trybie Normal Mode wyjście WO5
jest sterowane rejestrem CMP2).
Wartość porównania dla 50\% wypełnienia: $CMP2 = PER/2 = 249$.
...
\begin{enumerate}[nosep]
    ...
    \item Komparator AC0 porównuje sygnał z~progiem ustawionym przez
          ...
          Histereza $\pm\SI{25}{\milli\volt}$ (HYSMODE = 0x2) zabezpiecza
          przed wielokrotnym wyzwoleniem; tryb Fast Mode
          ($t_{PD} \approx \SI{50}{\nano\second}$, DS Tab.~36-32).
    \item Wyjście AC0 generuje zdarzenie w~EVSYS (kanał 0), które
...
\begin{table}[H]
    \centering
    \caption{Mapa pamięci EEPROM --- parametry kalibracyjne}
    \label{tab:eeprom_map}
    \begin{tabular}{llr}
        \toprule
        \textbf{Adres} & \textbf{Parametr} & \textbf{Rozmiar [B]} \\
        \midrule
        0x00 & Offset ToF ($t_{offset}$, int16) & 2 \\
        0x02 & Współczynnik korekcji $a$ (Q8.8 fixed-point) & 2 \\
        0x04 & Offset korekcji $b$ (int16, $\mu$m) & 2 \\
        0x06 & Częstotliwość $f_0$ skalibrowana (uint16, Hz) & 2 \\
        0x08 & Próg detekcji AC0 (uint8, wartość DAC) & 1 \\
        0x09 & Suma kontrolna CRC-8 & 1 \\
        \midrule
        & \textbf{Razem:} & \textbf{10} \\
        & Wolne: & 246 \\
        \bottomrule
    \end{tabular}
\end{table} Szacowane zapotrzebowanie na EEPROM:
...
\begin{table}[H]
    ...
    \begin{tabular}{lrrrr}
        ...
        \midrule
        ATtiny1616 @ 20\,MHz, 3{,}3\,V
            & ${\sim}\SI{6}{\milli\ampere}$
            & \SI{0,7}{\micro\ampere}
            & \SI{5}{\milli\second}
            & DS Tab.~36-4 (ekstrap.\ z~10\,MHz/3V: 3{,}1\,mA $\times 2$) \\
...
\begin{equation}
    I_{avg,1} = 6 + 0{,}1 + 0{,}002 + 20 \cdot 0{,}04 + 3{,}3 \cdot 0{,}2 \approx \SI{7,5}{\milli\ampere}
\end{equation}
\begin{equation}
    P_{in,1} = \frac{I_{avg,1} \cdot V_{3,3}}{\eta_{LDO}} = \frac{7{,}5 \cdot 3{,}3}{0{,}66} = \SI{37,5}{\milli\watt}, \quad
    P_{pkt,1} = 100/37{,}5 \approx \textbf{2,7~pkt}
\end{equation}
...
\begin{equation}
    ...
    = \frac{5 \cdot 6000 + 95 \cdot 0{,}7}{100} \approx \SI{301}{\micro\ampere}
    \label{eq:imcu_avg}
\end{equation}

Średni prąd całkowity (@ \SI{3,3}{\volt}):
\begin{equation}
    I_{avg,2} = 301 + 100 + 1{,}6 + 20000 \cdot \frac{0{,}2}{100} + 3300 \cdot \frac{1}{100}
    \approx \SI{476}{\micro\ampere}
\end{equation}

Moc pobierana z~zasilania \SI{5}{\volt}:
\begin{equation}
    P_{in,2} = \frac{I_{avg,2} \cdot V_{3,3}}{\eta_{LDO}} = \frac{0{,}476 \cdot 3{,}3}{0{,}66}
    \approx \SI{2,4}{\milli\watt}, \quad
    P_{pkt,2} = 100/2{,}4 \approx \textbf{42~pkt}
    \label{eq:moc_scen2}
\end{equation}
...
\begin{table}[H]
    ...
    \begin{tabular}{lrrr}
        ...
        \midrule
        Błąd RMSE         & 100 & 70  & 95 \\
        Średni pobór mocy & 50  & 42  & 50 \\
        Koszt elementów   & 50  & 50  & 50 \\
        Jakość projektu   & 100 & 65  & 90 \\
        \midrule
        \textbf{RAZEM}    & \textbf{300} & \textbf{227} & \textbf{285} \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Push-pull sterowanie nadajnika} --- wykorzystanie dwóch pinów
          GPIO w~przeciwfazie (PA5 przez TCA0-WO5 + PB5 sterowany
          ...
          (prąd wyjściowy GPIO: $I_{OH}/I_{OL} \leq \SI{20}{\milli\ampere}$,
          DS Tab.~36-3 --- wystarczający dla typowego nadajnika US), co dodatkowo
    ...
    \item \textbf{Automatyczna regulacja wzmocnienia (AGC)} --- wykorzystanie
          ...
          odległości. DAC0 może również sterować offsetem toru analogowego
          przez wyjście na pin PA6 (jedyny pin z~wyjściem DAC0, DS Tab.~31-2).
