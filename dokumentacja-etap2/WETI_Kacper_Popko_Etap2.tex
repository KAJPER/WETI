...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{llX}
		...
		Pobór mocy & $100/P_{[\si{\milli\watt}]}$ & zmierzone: spocz.\ \SI{0,9}{\milli\watt} $\to$ 50~pkt (sek.~\ref{sec:moc}) \\
...
\subsection{Zmontowany prototyp}
\label{sec:proto}

\begin{figure}[H]
	\centering
	\begin{minipage}{0.46\textwidth}\centering
		\includegraphics[height=7.2cm]{foto/plytka_gora.jpg}
		\\{\small Zmontowana płytka (widok z~góry)}
	\end{minipage}\hfill
	\begin{minipage}{0.46\textwidth}\centering
		\includegraphics[height=7.2cm]{foto/obudowa_front.jpg}
		\\{\small Urządzenie w~obudowie}
	\end{minipage}
	\caption{Zmontowany i~uruchomiony prototyp miernika.}
	\label{fig:proto}
\end{figure}
...
Zmierzony pobór w~spoczynku (Standby + RTC, tor analogowy odcięty Q1) wynosi
\SI{0,18}{\milli\ampere} przy \SI{5}{\volt}, czyli moc spoczynkowa
$P_{idle}=\SI{0,9}{\milli\watt}$. Pomiar aktywny trwa krótko
(${\sim}\SI{70}{\milli\second}$), pobierając \SIrange{2}{4,5}{\milli\ampere}.

\begin{table}[H]
	\centering
	\caption{Bilans prądowy (zmierzony multimetrem, \SI{5}{\volt})}
	\label{tab:prad}
	\begin{tabularx}{\textwidth}{Xrr}
		\toprule
		\textbf{Stan} & \textbf{Prąd} & \textbf{Moc} \\
		\midrule
		Spoczynek (Standby + RTC, op-amp off) & \SI{0,18}{\milli\ampere} & \SI{0,9}{\milli\watt} \\
		Pomiar aktywny (op-amp + MCU + ADC) & \SIrange{2}{4,5}{\milli\ampere} & \SIrange{10}{22,5}{\milli\watt} \\
		\bottomrule
	\end{tabularx}
\end{table}

Przy pracy na żądanie (krótki pomiar co kilka sekund) średnia jest zdominowana
przez spoczynek: np.\ \SI{70}{\milli\second} pomiaru @ \SI{3}{\milli\ampere} co
\SI{10}{\second} daje $I_{avg}\approx\SI{0,2}{\milli\ampere}$, czyli
$P_{avg}\approx\SI{1}{\milli\watt}$. Nawet sam pobór spoczynkowy
($\SI{0,9}{\milli\watt}$) daje
$P_{pkt}=\min(50,\ 100/0{,}9)=\SI{50}{\pkt}$ --- maksymalną punktację
w~kryterium mocy.
...
\begin{figure}[H]
	...
	\begin{axis}[
		...
		xlabel={Odległość rzeczywista [mm]},
		ylabel={Odczyt [mm]},
		xmin=80, xmax=520, ymin=80, ymax=520,
		grid=both, grid style={gray!20},
		legend pos=north west, legend cell align=left,
		]
		% linia idealna y=x
		\addplot[dashed, gray, thick, domain=80:520] {x};
		\addlegendentry{idealna $y=x$}
		% zmierzone punkty z error-barami (sigma)
		\addplot[only marks, mark=*, mark size=1.6pt, color=NavyBlue,
		...
		\addlegendentry{zmierzone $\pm\sigma$}
	\end{axis}
	\end{tikzpicture}
	\caption{Charakterystyka pomiarowa po kalibracji ($M=35$). Punkty pokrywają
		się z~linią idealną; $R^2>0{,}999$.}
	\label{fig:char}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{axis}[
		...
		xlabel={Odległość rzeczywista [mm]},
		ylabel={Residuum $D-D_{ref}$ [mm]},
		xmin=80, xmax=520, ymin=-5, ymax=5,
		grid=both, grid style={gray!20},
		ybar, bar width=10pt,
		]
		\addplot[fill=NavyBlue!60, draw=NavyBlue] coordinates {
			(100,0.0)(150,2.3)(200,-2.0)(250,0.3)(300,-0.4)(400,-1.0)(500,-0.4)
		};
		\draw[red, thick] (axis cs:80,2.4) -- (axis cs:520,2.4);
		\draw[red, thick] (axis cs:80,-2.4) -- (axis cs:520,-2.4);
	\end{axis}
	\end{tikzpicture}
	\caption{Residua pomiaru względem wzorca; linie czerwone --- poziom
		$\pm\text{RMSE}=\pm\SI{2,4}{\milli\metre}$.}
	\label{fig:resid}
\end{figure}
